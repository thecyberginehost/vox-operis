...
CREATE POLICY IF NOT EXISTS "Avatar files are publicly accessible" ON storage.objects
...
-- Update RLS policy for profiles table to allow users to update their own profiles
CREATE POLICY IF NOT EXISTS "Users can update own profile" ON profiles
FOR UPDATE USING (auth.uid() = id);

-- Create/update RLS policy for profiles table to allow users to view their own profiles
...
-- Add some sample specialties data (optional)
